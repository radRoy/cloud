...
# below part is (still incomplete btw, TBD) for when resources get allocated on another day than when they where requested. The problem is calling the right file with $session or $checkdir, which contain the date of requesting resources.
